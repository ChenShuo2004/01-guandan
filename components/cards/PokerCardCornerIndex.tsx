...
export function PokerCardCornerIndex({ card, isLevelCard, size, variant }: PokerCardCornerIndexProps) {
  ...
  const rankSize = Math.round((variant === "played" ? 17 : size === "sm" ? 18 : 21) * scale);
  const suitSize = Math.round((variant === "played" ? 14 : size === "sm" ? 16 : 18) * scale);
  ...
  return (
    <div
      aria-hidden="true"
      className={cn(
        "pointer-events-none absolute left-[5px] top-[5px] z-20 flex flex-col items-center rounded-[8px] px-[7px] py-[5px]",
        variant === "played" ? "min-w-[22px]" : "min-w-[34px]",
        isLevelCard
