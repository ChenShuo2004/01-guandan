...
export function detectCardPattern(cards: Card[]): CardPattern {
  ...
  if (cards.length === 0) return invalid(cards, "请选择要出的牌");
  if (cards.length === 1) return valid("single", highestRank, sortedCards);
  if (cards.length === 2 && rankCounts.size === 1) return valid("pair", highestRank, sortedCards);
  if (cards.length === 3 && rankCounts.size === 1) return valid("triple", highestRank, sortedCards);
  if (isFourJokers(sortedCards)) return valid("fourJokers", 1000, sortedCards);
...
function valid(type: CardPatternType, power: number, cards: Card[]): CardPattern {
  return { valid: true, type, power, cards };
}

function invalid(cards: Card[], message: string): CardPattern {
  return { valid: false, type: "invalid", power: 0, cards, message };
}
